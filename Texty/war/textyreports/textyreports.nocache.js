...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "textyreports" because\n';
  ...
  $wnd.__gwt_activeModules['textyreports'] = {
    'moduleName' : 'textyreports',
    'bindings' : function() {
      return {};
    }
  };
 ...
 * "/textyreports.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['textyreports'].superdevmode = true;

  var expectedSuffix = '/textyreports.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + textyreports + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:textyreports';
  var devModeSessionKey = '__gwtDevModeSession:textyreports';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/textyreports/textyreports.nocache.js';
  ...
  $wnd.__gwt_activeModules['textyreports'].canRedirect = true;
  $wnd.__gwt_activeModules['textyreports'].superdevmode = true;
  ...
  css = (moduleIdx == 1 ? css : '') +
    ".gwt-DevModeModule-" + moduleIdx + ".gwt-DevModeCompiling div:before{" +
      "content:'COMPILING textyreports';" +
      "font-size:24px;" +
      "color:#d2d9ee;" +
    "}";
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/textyreports?user.agent=' + ua + '&_callback=' + callback;
